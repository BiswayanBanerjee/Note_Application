...
export const signup = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [registeredUser]: any = await mysqlPool.query(
    ...
    if (existingUser.length > 0) {
      ...
      res
        .status(200)
        .json({
          message: "OTP resent to email. Please verify to complete signup.",
        });
      return;
    }
...
export const sendOTP = async (req: Request, res: Response) => {
  ...
  try {
    const otp = generateOTP();
    await mysqlPool.query("INSERT INTO otps (email, otp) VALUES (?, ?)", [
      email,
      otp,
    ]);
...
export const verifyOTP = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await mysqlPool.query("DELETE FROM otps WHERE email = ?", [email]);
  } catch (error) {
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await mysqlPool.query("UPDATE users SET password = ? WHERE email = ?", [
      hashedPassword,
      email,
    ]);
...
export const verifySignupOTP = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [result]: any = await mysqlPool.query(
      "INSERT INTO users (email, password, name, date_of_birth, provider) VALUES (?, ?, ?, ?, 'local')",
    ...
    // Cleanup
    await mysqlPool.query("DELETE FROM unverified_users WHERE email = ?", [
      email,
    ]);

    // Generate JWT with CORRECT ID
    const token = jwt.sign(
...
export const googleLogin = async (req: Request, res: Response) => {
  try {
    ...
    if (!email) {
      res.status(400).json({ message: "Invalid Google account" });
      return;
    }
